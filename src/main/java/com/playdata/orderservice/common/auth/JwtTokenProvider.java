...
// yml에 있는 값 가져오기(properties 방식으로 지목)
...
public class JwtTokenProvider {
    ...
    public String createToken(String email, String role) {
        ...
        return Jwts.builder().
                setClaims(claims)
                .setIssuedAt(now)
                //현재 시간을 밀리초로 바꾼 후 30분 후를 만료시간으로 세팅
    ...
    public String createRefreshToken(String email, String role) {
        ...
        return Jwts.builder().
                setClaims(claims)
                .setIssuedAt(now)
                //토큰 생성 로직은 같지만 수명과 서명이 다릅니다. (Refresh가 좀 더 길어요, 서명도 원래는 다르게 세팅)
    ...
    public TokenUserInfo validateAndGetTokenUserInfo(String token) throws Exception {
        ...
        return TokenUserInfo.builder()
                ...
                //일단 String으로 데이터를 꺼내고 직접 Role 타입으로 포장해서 넣어 줍니다.
